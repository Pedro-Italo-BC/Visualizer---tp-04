# Avaliação com Usuários

## Roteiro de Teste

1. Cadastre uma entidade chamada Produto, contendo os campos:
- Código: 001
- Nome: Notebook
- Preço: 3500
2. Verifique se a entidade aparece na visualização dos arquivos.
3. Consulte o registro criado e identifique em qual posição ele foi armazenado.
4. Atualize o preço do produto para R$ 4000.
5. Verifique se a alteração foi refletida na visualização dos dados.
6. Cadastre mais dois produtos:
- Código: 002, Nome: Mouse, Preço: 80
- Código: 003, Nome: Teclado, Preço: 150
7. Exclua o produto de código 002.
8. Verifique como o sistema representa o registro excluído.
9. Navegue pelas diferentes áreas da aplicação e explore as visualizações disponíveis.

## Resultado da Avaliação (Escala Likert 1–5)

### Pontuações Individuais

| Avaliador | Q1 | Q2 | Q3 | Q4 | Q5 | Q6 | Q7 | Média |
|---|:---:|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Daniel Rocha | 5 | 4 | 5 | 5 | 5 | 5 | 5 | **4,86** |
| Daniel Santos Garcia | 5 | 5 | 5 | 5 | 5 | 5 | 5 | **5,00** |
| Davi Martins | 5 | 5 | 5 | 5 | 5 | 5 | 5 | **5,00** |
| Isabel Cristina | 5 | 5 | 5 | 5 | 5 | 5 | 4 | **4,86** |
| Laura Bargas | 5 | 5 | 5 | 5 | 4 | 5 | 4 | **4,71** |
| Lucca de Paula | 5 | 5 | 5 | 5 | 5 | 5 | 5 | **5,00** |
| Pedro Augusto Gomes | 4 | 5 | 5 | 5 | 5 | 5 | 5 | **4,86** |
| Pedro Mattar | 5 | 4 | 5 | 5 | 5 | 4 | 5 | **4,71** |
| Rafael Henrique Silva | 5 | 5 | 5 | 4 | 5 | 5 | 5 | **4,86** |
| Yuri Penido | 5 | 5 | 5 | 5 | 5 | 4 | 4 | **4,71** |

### Médias por Questão

| Item | Afirmação | Média |
|---|---|:---:|
| 1 | A interface é fácil de aprender e utilizar. | **4,90** |
| 2 | As funções principais são fáceis de localizar. | **4,80** |
| 3 | Os elementos da interface possuem nomes e descrições claras. | **5,00** |
| 4 | As mensagens exibidas pelo sistema são úteis e compreensíveis. | **4,90** |
| 5 | Foi possível concluir as tarefas sem auxílio externo. | **4,90** |
| 6 | A aplicação me ajudou a compreender como os registros são armazenados em arquivos. | **4,80** |
| 7 | Estou satisfeito(a) com a experiência geral de uso da aplicação. | **4,70** |
| | **Média Geral** | **4,86** |

## Conclusão

A aplicação obteve média geral de 4,86/5,0, indicando elevada satisfação dos participantes. Os usuários destacaram principalmente a facilidade de uso da interface e a contribuição da ferramenta para a compreensão do armazenamento de registros em arquivos.
